<!DOCTYPE html>
<html lang="en">
<head>
    <?php include "header-title.php"; ?>
</head>
<body>
    <?php include "header.php"; ?>
    <main id="location">
        <div class="banner-section position-relative text-white" style="background-image: url(styles/images/location/banner-1.webp);">
            <div id="move-trigger"></div>
            <div id="move-end-trigger"></div>
            <div class="move-element">
                <img class="logo d-none d-sm-table mx-auto" src="styles/images/common/logo-2.svg" alt="">
                <h1 class="title d-table fs-170 fw-bolder mx-auto">LOCATION</h1>
            </div> 
            <img class="building position-absolute" src="styles/images/location/banner-2.webp" alt="" >
            <div class="scroll d-none d-xl-block fs-14 fw-bold text-center">
                <span class="d-block">SCROLL</span>
                <img src="styles/images/common/scroll.svg" alt="">
            </div>
        </div>
        <div class="section-1 text-white position-relative">
            <div class="text-block" >
                <img src="styles/images/location/section-1-text.svg" alt="" data-aos="fade-up">
            </div>
            <div class="main-block">
                <div class="title-box text-center z-1"  data-aos="fade-up">
                    <img class="title" src="styles/images/location/1-logo.svg" alt="">
                    <p class="fs-48 fw-bold lh-12">
                        <span class="d-block">內科 南軟 信義</span>
                        <span class="d-block">萬中選一</span>
                    </p>
                </div>
                <div class="row row-cols-lg-2 g-0">
                    <div class="box position-relative">
                        <img class="h-100" src="styles/images/location/1-1.webp" alt="">
                        <div class="text">
                            <p class="title fs-32 fw-bold" data-aos="fade-up">台北產經金三角</p>
                            <p class="fs-18 lh-17" data-aos="fade-up">
                                <span class="d-block">内科南軟中軸線，製造業、科技業、金融業掌心地塊。</span>
                                <span class="d-block">一橋之隔近取信義計畫、環東大道交流道旁，石潭公園1935坪稀土角地、決勝拍板。</span>
                            </p>
                        </div>
                    </div>
                    <div class="box position-relative">
                        <img class="h-100" src="styles/images/location/1-2.webp" alt="">    
                        <div class="text">
                            <p class="title fs-32 fw-bold" data-aos="fade-up">內湖萬有引力 百大總部磁王</p>
                            <p class="fs-18 lh-17" data-aos="fade-up">威剛總部入主、萬海航運列席、三商美邦定址、浩鑫購地自建總部，中國信託科技行政雙中心。新一代總部換日線『興富發』定址內湖核心、聚峰為首。</p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <div class="section-2 position-relative">
            <div class="text-block position-relative d-table z-2 mx-auto">
                <img src="styles/images/location/section-2-text.svg" alt="" data-aos="fade-up">
            </div>
            <div class="main-block d-lg-flex text-white">
                <div class="swiper-box-1" data-aos="fade-up">
                    <div class="swiper h-100">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-1.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-2.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-3.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-4.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-5.webp" alt="">
                            </div>
                        </div>
                    </div>
                    <div class="d-flex justify-content-center mt-4 mt-sm-0">
                        <div class="swiper-button swiper-button-prev bg-hover-golden d-sm-none mx-2">
                            <i class="icon-left"></i>
                        </div>
                        <div class="swiper-button swiper-button-next bg-hover-golden d-lg-none mx-2">
                            <i class="icon-right"></i>
                        </div>
                    </div>
                    <div class="swiper-pagination fw-600 fs-15 d-none d-sm-block"></div>
                </div>
                <div class="text-box">
                    <p class="fs-48 fw-bold" data-aos="fade-up">
                        <span class="d-block d-sm-inline">以一為名</span>
                        <span class="d-block d-sm-inline">優勢成雙</span>
                    </p>
                    <div class="swiper-text" data-aos="fade-up">
                        <div class="swiper">
                            <div class="swiper-wrapper">
                                <div class="swiper-slide">
                                    <p class="fs-32 fw-bold">雙園區 正中軸</p>
                                    <p class="fs-20">
                                        <span class="d-block">台北科技產業軸帶核心</span>
                                        <span class="d-block">串聯內科、南港經貿園區</span>
                                        <span class="d-block">掌門大內科關鍵聚落</span>
                                    </p>
                                </div>
                                <div class="swiper-slide">
                                    <p class="fs-32 fw-bold">雙國道 暢全台</p>
                                    <p class="fs-20">
                                        <span class="d-block">鄰國道一號內湖交流道</span>
                                        <span class="d-block">環東大道南北串聯</span>
                                        <span class="d-block">接國道3號北二高 暢行全台</span>
                                    </p>
                                </div>
                                <div class="swiper-slide">
                                    <p class="fs-32 fw-bold">雙跨橋 大東區</p>
                                    <p class="fs-20">
                                        <span class="d-block">成功橋 抵捷運昆陽站</span>
                                        <span class="d-block">成美橋 達松山火車站</span>
                                        <span class="d-block">大信義東區 讀秒現身</span>
                                    </p>
                                </div>

                                <div class="swiper-slide">
                                    <p class="fs-32 fw-bold">雙捷匯 補漲席</p>
                                    <p class="fs-20">
                                        <span class="d-block">環狀民生汐止線必經要道</span>
                                        <span class="d-block">民汐SB09站約750 公尺</span>
                                        <span class="d-block">環狀線Y35舊宗近距離</span>
                                    </p>
                                </div>
                                <div class="swiper-slide">
                                    <p class="fs-32 fw-bold">雙臨路 大基地</p>
                                    <p class="fs-20">
                                        <span class="d-block">安康路黃金角地1935坪</span>
                                        <span class="d-block">面16800坪石潭公園</span>
                                        <span class="d-block">大器地格 永恆棟距</span>
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="swiper-box-2 d-none d-lg-block" data-aos="fade-up">
                    <div class="swiper h-100">
                        <div class="swiper-wrapper">
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-2.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-3.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-4.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-5.webp" alt="">
                            </div>
                            <div class="swiper-slide">
                                <img src="styles/images/location/2-1.webp" alt="">
                            </div>
                        </div>
                    </div>
                    <div class="swiper-button swiper-button-next bg-hover-golden">
                        <i class="icon-right"></i>
                    </div>
                    <div class="swiper-pagination fw-600 fs-15"></div>
                </div>
            </div>
        </div>
    </main>
    <?php include "contact.php"; ?>
    <?php include "footer.php"; ?>
    <?php include "footer-js.php"; ?>
</body>
</html>